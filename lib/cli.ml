...
module Arguments = struct
  let dry_run =
    ...
    let doc =
      "Print description of what would happen if $(tname) were run in normal \
       mode."
    in
    let docv = "D" in
  ...
  let print_config =
    let open Cmdliner.Arg in
    let doc =
      "Print the default configuration to stdout so that it can be redirected \
       to a config file for the user to customize."
    in
    let docv = "P" in
...
module Command = struct
  ...
  let manpage_info =
    let description =
      "$(tname) creates a skeleton for a library-executable OCaml project that \
       assumes the UChicago Library's opam repository and standard libraries."
    in
