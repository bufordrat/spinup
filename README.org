...
- includes configuration to serve a CGI binary locally with =althttpd=
- uses Docker/Alpine Linux to build a fully statically-linked =musl=
  binary out of the box, runnable on anything with a Linux kernel
- unit tests and cram tests run inside the Docker environment, to
